...
def test_game_release_date_setter():
    ...
    assert game.release_date_formatted() == "Oct 21, 2008"
    with pytest.raises(ValueError):
...
def test_game_add_review():
    # test game without any reviews
    game1 = Game(1, "game1")
    assert len(game1.reviews) == 0

    # test garbage inputs
    assert game1.add_review("haha") is False
    assert len(game1.reviews) == 0

    # test valid inputs
    assert game1.add_review(Review(User("user1", "password1"), game1, 3, "comment1")) is True
    assert len(game1.reviews) == 1
    assert game1.add_review(Review(User("user2", "password2"), game1, 3, "comment2")) is True
    assert len(game1.reviews) == 2

    # test duplicate inputs
    assert game1.add_review(Review(User("user2", "password2"), game1, 3, "comment2")) is False
    assert len(game1.reviews) == 2
...
def test_read_csv_file():
    ...
    assert game.release_date_formatted() == "Nov 12, 2007"
